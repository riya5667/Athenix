          ...
          <RoughNotationGroup show={isInView}>
            ...
            >
              ...
              <RoughNotation
                ...
                multiline
              >
                <span className="text-currentColor">event</span>
              </RoughNotation>{" "}
              with ease and{" "}
              <RoughNotation
                type="underline"
            ...
            <p className="text-neutral-500 dark:text-neutral-400 text-sm md:text-lg max-w-2xl mt-4 md:mt-10 text-center sm:text-left">
              EventMaster helps you organize conferences, weddings, and corporate events effortlessly. Input your event details, and we'll suggest the best{" "}
              <RoughNotation
                type="underline"
                animationDuration={2000}
                iterations={3}
                color="#F3D0D7"
              >
                vendors, venues, and services
              </RoughNotation>{" "}
              tailored to your needs. Manage budgets, schedules, and invitations all in one place.
            </p>
          </RoughNotationGroup>
          <div className="flex sm:flex-row flex-col gap-4 items-center mt-10 [perspective:800px]">
            <button className="px-4 py-2 rounded-lg bg-[#F3D0D7] w-full sm:w-auto font-bold text-black text-base hover:[transform:rotateX(10deg)] transition duration-200 origin-left hover:shadow-lg">
              Start Planning
            </button>
            <button className="text-black dark:text-white hover:border-[#F3D0D7] border border-transparent px-4 py-2 rounded-lg text-base transition duration-200">
              Learn More
            </button>
          </div>
        </div>
...
export const EventPlannerMockup = () => {
  const ref = useRef(null);
  const isInView = useInView(ref);
  useEffect(() => {
    const sequence = [
    ...
    <div ref={ref} className="relative pt-20 w-[360px] h-[600px] m-auto">
      ...
      <div className="px-8 mt-0 md:mt-10 flex flex-col gap-4 relative z-20">
        <div className="first opacity-0 text-sm text-neutral-800 dark:text-neutral-100 p-2 bg-gray-100 dark:bg-neutral-700 rounded-lg">
          Welcome to EventMaster! Let's plan your event.
        </div>
        ...
        <div className="features grid grid-cols-2 gap-2">
          <div className="feature opacity-0 text-xs p-2 bg-yellow-100 dark:bg-yellow-800 rounded-lg text-center">Conference</div>
          <div className="feature opacity-0 text-xs p-2 bg-blue-100 dark:bg-blue-800 rounded-lg text-center">Wedding</div>
          <div className="feature opacity-0 text-xs p-2 bg-green-100 dark:bg-green-800 rounded-lg text-center">Corporate Event</div>
          <div className="feature opacity-0 text-xs p-2 bg-purple-100 dark:bg-purple-800 rounded-lg text-center">Other</div>
        </div>
        <div className="features mt-4">
          <div className="feature opacity-0 text-xs p-2 bg-gray-200 dark:bg-gray-700 rounded-lg mb-2">📅 Date & Time</div>
          <div className="feature opacity-0 text-xs p-2 bg-gray-200 dark:bg-gray-700 rounded-lg mb-2">📍 Location</div>
          <div className="feature opacity-0 text-xs p-2 bg-gray-200 dark:bg-gray-700 rounded-lg">👥 Guest List</div>
        </div>
      </div>
    </div>
  );
